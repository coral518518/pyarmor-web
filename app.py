from fastapi import FastAPI, Request, HTTPException, Query, UploadFile, File, BackgroundTasks
...
async def obfuscate(
    ...
    format: str = Query("text", regex="^(text|zip)$"),
    platform: str = Query(None, description="Target platforms, e.g. windows.x86_64,linux.x86_64")
):
    uid = str(uuid.uuid4())
    ...
    if "multipart/form-data" in content_type:
        ...
        if not body:
            raise HTTPException(status_code=400, detail="Empty request body. Please POST Python code.")
        ...
        if not code:
            raise HTTPException(status_code=400, detail="No Python code content found in the request body")
    ...
    if not os.path.exists(dist_file):
        cleanup_dir(workdir)
        raise HTTPException(status_code=500, detail="Obfuscated file was not generated correctly.")
    ...
    if format == "text":
        ...
        cleanup_dir(workdir)
        return PlainTextResponse(obfuscated_code)
        
    else:  # ZIP output format containing both the obfuscated script and the runtime package
